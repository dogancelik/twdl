	...
	return rp(tweetUrl)
		...
		.then(jq => {
			let isVideo = jq('meta[property="og:type"][content="video"]').length > 0,
				name = jq('.permalink-header .fullname').first().text(),
	...
	if (options.embed) {
		try {
			if (filename.includes('.mp4')) {
				await exiftool.write(filename, { Comment: [
					tweetUrl, mediaUrl, mediaData.text,
					mediaData.username, mediaData.name,
					mediaData.userId, mediaData.dateFormat
				].join('\n') }, exifArgs);
			} else {
				await exiftool.write(filename, {
					Credit: tweetUrl, Source: mediaUrl, ObjectName: mediaData.text,
					"By-line": mediaData.username, "By-lineTitle": mediaData.name,
					CopyrightNotice: mediaData.userId, DateCreated: mediaData.dateFormat
				}, exifArgs);
			}
