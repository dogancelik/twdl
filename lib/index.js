...
function downloadUrl(mediaUrl, tweetUrl, options) {
	...
	let prePr = parsedPath.dir ? mkdirp(parsedPath.dir).then(pr) : pr;

	return options.source ?
		prePr
			.then(() => filename.includes('.mp4')
				? exiftool.read(filename, exifArgs.concat(['-api', '-Title '+tweetUrl, '-Description '+mediaUrl]))
				: exiftool.write(filename, { Credit: tweetUrl, Source: mediaUrl }, exifArgs))
			.then(
