...
function downloadUrls(urls, options) {
	...
	return Promise.each(urls, function (tweetUrl, index, length) {
		...
		return Promise.join(
			...
			function (userId, mediaData, videoUrl) {
				...
				if (videoUrl && mediaData.isVideo) {
					mediaCount += 1;
					mediaData.media.push(videoUrl);
				}
				logFound(mediaCount);

				return Promise.all(mediaData.media.map((mediaUrl) => downloadUrl(mediaUrl, tweetUrl, mediaData, options)));
			}).then(function () {
