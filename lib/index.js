...
function getVideo(tweetUrl) {
	return rp({ uri: 'https://twdown.net/download.php', method: 'POST', form: { URL: tweetUrl } })
		.then(html => cheerio.load(html), downloadError)
		.then(jq => {
			let rows = jq('a[download]').first().parent().parent().parent().children(),
				result = { width: 0, height: 0, res: 0, link: null };

			rows.each(function (i, el) {
				if (i === rows.length - 1) return true; // skip last row

				let row = jq(el),
					[width, height] = row.children().eq(1).text().split('x'),
					res = parseInt(width, 10) * parseInt(height, 10);
